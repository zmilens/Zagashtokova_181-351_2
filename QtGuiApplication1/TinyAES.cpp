...
{
	if (p_input.isEmpty()) {
		qDebug() << "TinyAES::Encrypt(..): cannot encrypt empty input";
		return QByteArray();
	}
...
{
	if (p_input.isEmpty()) {
		qDebug() << "TinyAES::Decrypt(..): cannot decrypt empty input";
		return QByteArray();
	}
...
QByteArray TinyAES::Encrypt(QByteArray b_input, QByteArray p_key, QByteArray p_iv)
{
	if (b_input.isEmpty()) {
		qDebug() << "TinyAES::Encrypt(...): cannot encrypt empty input";
		return QByteArray();
	...
	if (keySize != 16 && keySize != 24 && keySize != 32) {
		qDebug() << "TinyAES::Encrypt(): INVALID KEYSIZE";
		return QByteArray();
	}

	if (ivSize != 16) {
		qDebug() << "TinyAES::Encrypt(): INVALID IVSIZE";
		return QByteArray();
	}
	...
	for (int part = 0; part < chunksCount; part++) {
		...
		if (part * 500000 > b_input.size()) {
			qDebug() << "TinyAES::Encrypt(): skipped: buffer overflow";
			continue;
		}
...
QByteArray TinyAES::Decrypt(QByteArray b_input, QByteArray p_key, QByteArray p_iv)
{
	if (b_input.isEmpty()) {
		qDebug() << "TinyAES::Decrypt(...): cannot decrypt empty input";
		return QByteArray();
